...
async function makePayloadHash(delegation) {
  ...
  let mapHash = await crypto.subtle.digest('SHA-256', concatBuffers(expirationHash, pubkeyHash));
  let payload = concatBuffers(new TextEncoder("utf-8").encode("\x1Aic-request-auth-delegation"), mapHash);
...
async function validate(delegation, challenge, signedChallenge, now) {
  try {
    ...
    if (!await ed.verify(
      ...
      toHex(extractDER(fromHex(delegation.delegations[0].delegation.pubkey), DER_ED25519_PREFIX)))) {
...
validate(delegation, challenge, signedChallenge, 0x1726ecb7ed82a83c).then(r => console.log('validate', r));
